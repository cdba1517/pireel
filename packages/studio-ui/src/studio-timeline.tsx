...
const SCENE_PAD_T = 12; // 场景卡距轨上缘的空隙(同剪映):从空隙起拖=框选,从卡面起拖=重排;上侧大一点好命中
...
/** 源时间锚定的胶片开窗(同剪映):第 k 格恒覆盖源时间 [k,k+1)×tileDur,取与窗口
...
interface StudioTimelineProps {
  ...
  /** 框选:鼠标在场景轨**上下空隙**起拖出矩形(同剪映;从卡面起拖=重排),命中的分镜 id 一次性设为多选集。 */
  onBoxSelectShots: (ids: string[]) => void;
  /** 场景卡拖动重排(从卡面起拖,同剪映):把片段序列里第 from 个挪到第 to 位(splice 语义)。 */
  ...
  /** 块拖进行间空隙=掰出新轨(同剪映):slot=新轨在叠加轨自上而下显示序里的插入位(0..N)。
...
function StudioTimelineImpl({
  ...
  // 缩率图方格(同剪映):正方形贴片(格宽=格高,object-cover 裁),网格锚在**源时间**上——
   ...
   *  卡面按下被卡自己拦下走重排(startShotDrag),两套手势按起点分流,同剪映。 */
  ...
  // 场景卡拖动重排(同剪映):从**卡面**起拖=调顺序,从轨上下空隙起拖=框选(onLanePointerDown)。
                  ...
                  {sceneSpans.map(({ shot, start, end }, i) => {
                    ...
                    return (
                      <div key={shot.id}>
                        <button
                          type="button"
                          // 从卡面按下=进重排通道(stopPropagation 拦掉轨面框选;上下空隙才是框选入口,同剪映)
                          onPointerDown={(e) => {
                            e.stopPropagation();
                            startShotDrag(e, i);
                          }}
